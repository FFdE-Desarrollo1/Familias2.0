...
Public Class AjustarExistenciasMM
    Public sMember As String
    Dim iMeds As Integer

    Private Sub AjustarExistenciasMM_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        BannerInfo()
        valoresIniciales()
    End Sub
    ...
    Private Sub checkCorrelativeMeds()
        ...
        For f = 0 To n
            ...
            If MedsGrid.Item(6, f).Value < 0 Then
                MedsGrid.Item(6, f).Style.BackColor = Color.Yellow
            End If
        Next
    End Sub
    ...
    Private Sub insertarMedicina(ByVal fech As String, ByVal codMed As String,
        ...
        sql = "INSERT INTO dbo.HealthInventoryTransaction VALUES ('F', '" + FechaP.Value.ToString(en.DateTimeFormat) + "', 'MATM', '" + codMed + "', GETDATE(), " + _
    ...
    Private Sub valoresIniciales()
        sql = "SELECT Code, CASE WHEN DosageDesc_es IS NOT NULL THEN (CAST(Code AS nvarchar(7)) + ' - ' + DescSpanish + '(' + DosageDesc_es + ')') " + _
                        ...
                        "FROM dbo.CdHealthInventoryItem WHERE Active = 1 AND ValidSelection = 1  AND Category = 'MATM' "
        bdCombo(sql, MedsCombo, "Code", "Medicine")

        sql = "SELECT Code, DescSpanish Descripcion FROM dbo.CdInventoryAdjustmentReason WHERE Active = 1 AND Program IN ('***', 'ENFE') ORDER BY DescSpanish "
