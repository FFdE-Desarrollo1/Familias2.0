...
Public Class RecetasExternasFarm
    Dim affilStatus As String
    Dim memberId, iMeds, edad As Integer
    Dim orTotal, hPercent, orPercentage, costExam As Double
    Dim conEntrega As Boolean = False

    Private Sub RecetasExternas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        BannerInfo()
    ...
    Private Sub GuardarButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GuardarButton.Click
        ...
        guardarListadoMeds()
        Dim nv As New HistorialMedicinasMiembroFarm
        nv.PacienteV.Text = PacienteV.Text
        nv.Show()
        Me.Close()
    End Sub
    ...
    Private Sub guardarListadoMeds()
        If MedsGrid.Rows.Count() >= 1 Then
            If Len(DoctorC.Text) = 0 Then
                MsgBox("Necesita selección al médico o terapeuta que autorizó la receta" + normalTotalV.Text, vbInformation, My.Resources.versionFamilias2)
            Else
    ...
    Private Sub historial()
        'fill meds history
        sql = "SELECT Prescripción, Entrega, Medicina, Cantidad * -1 Cantidad, ApprovedTotalPrice Q_Aprobado,  Observaciones, Indications Indicaciones, Autorizado, EntregadoPor " + _
    ...
    Private Sub insertarMedicina(ByVal fech As String, ByVal codMed As String, ByVal cant As String,
        ...
        If Double.Parse(aTpri) > 0 And ExonerarCheck.Checked = False Then
            entregado = ", NULL, NULL "
        Else
            entregado = ", '" + fech + "', '" + U + "'"
        End If
        ...
        '    ElseIf 
        'End If
        'If aTpri = "0.00" Or aTpri = "0" Or exon = "1" Then
        '    entregado = ", '" + fech + "', '" + U + "'"
        '    'ElseIf conEntrega = True Then
        '    '    entregado = ", '" + fech + "', '" + U + "'"
        'Else
        '    entregado = ", NULL, NULL "
        'End If
        ...
        sql = "INSERT INTO dbo.HealthInventoryTransactionMedi VALUES (" + tranId.ToString + ", GETDATE(), ' ', '" + U + "', NULL, 'F', " + _
         "'" + fech + "', '" + DoctorC.Text + "', '" + codMed + "', " + (cant * -1).ToString + ", 'FARM', '" + S + "', " + PacienteV.Text + ", " + uPri + ", " + aTpri + _
         ", '" + indi + "', '" + obse + "', NULL, NULL" + entregado + ", 0, " + exon + ", 'REXT')"
        bdInsertar(sql)
    End Sub
    ...
    Private Sub llenarMedsGridConPrecio(ByVal precio As String)
        Dim price, totalByMed, apr As Double
        Double.TryParse(precio, price)
        totalByMed = price * Convert.ToDouble(CantidadV.Text)
        apr = totalByMed * (Double.Parse(normalPercentageV.Text) / 100)
                ...
                Else
                    If suficienteExistencia(MedsCombo.SelectedValue, CantidadV.Text, MedsCombo.Text) = True Then 'check actually quantity
                        If MedsCombo.SelectedValue = "OTRA" Then
                            Dim precio As String = InputBox("VALOR UNITARIO DE LA MEDICINA " + vbCrLf + "(Si desconoce el precio solo presione ENTER, colocará automáticamente 0)", My.Resources.versionFamilias2, "Por favor Ingrese el valor de la medicina. Ej. 0.25")
                            llenarMedsGridConPrecio(precio)
                            iMeds = iMeds + 1
                        Else
                            llenarMedsGrid()
                            iMeds = iMeds + 1
    ...
    Private Sub valoresIniciales()
        ...
        sql = "SELECT M.FirstNames + ' ' + M.LastNames Nombre, dbo.fn_GEN_CalcularEdad(M.BirthDate) AS Edad, M.LastFamilyId Familia, F.Classification Clasificacion, " + _
        ...
        EdadV.Text = " - " + chequearValor(genInfo.Rows(0)("Edad"))
        ...
        sql = "SELECT EmployeeId FROM FwEmployeeRole WHERE Status = 'ACTV' AND Role IN ('MEDI', 'TERA') AND Organization = '" + S + "'  GROUP BY EmployeeId"
